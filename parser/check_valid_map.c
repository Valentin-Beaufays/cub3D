...
static int check_valid_map_char(char **map)
{
    ...
    while (map[y])
    {
        ...
        while (map[y][x])
        {
            if (map[y][x] != '1' && map[y][x] != '0' && map[y][x] != '2' && map[y][x] != 'N' && map[y][x] != 'S' && map[y][x] != 'E' && map[y][x] != 'W' && map[y][x] != ' ')
                return (0);
            x++;
        }
        y++;
...
static int check_valid_line(char *line)
{
    ...
    while (line[x])
    {
        if ((in == 0 && (line[x] == '0' || line[x] == '2')) || (in == 2 && line[x] == ' '))
            return (0);
...
static int check_valid_col(char **map, size_t x)
{
    ...
    while (map[y])
    {
        ...
        if ((in == 0 && (map[y][x] == '0' || map[y][x] == '2')) || (in == 2 && map[y][x] == ' '))
            return (0);
